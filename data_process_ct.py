...
class MultiModalDataset(Dataset):
    def __init__(
        ...
        self.labels = torch.nn.functional.one_hot(torch.tensor([s["label"] for s in valid_samples]).long()).float()
        # self.modal_mask_transform = RandomMaskModalTransform(mask_prob=0.1) if self.mode == "train" else None
        self.modal_mask_transform =  None
    ...
    def __getitem__(self, idx: int) -> Tuple[Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor], torch.Tensor]:
        ...
        ct_data = np.load(sample["ct_path"])
        if self.mode == "train":
            # data = self.rotate(data)
            ct_data = self.transform(ct_data)
        # 转为 float32（PyTorch 常用）        
        ct_tensor = ct_data.astype(np.float32)
        ct_tensor = np.expand_dims(ct_tensor, 0)
        if isinstance(ct_tensor, np.ndarray):
                ct_tensor = torch.from_numpy(ct_tensor)
        elif hasattr(ct_tensor, 'as_tensor'):
                ct_tensor = ct_tensor.as_tensor().clone().detach()
        ...
        if self.mode == "train" and self.modal_mask_transform:
            ct_tensor, _, _, modal_mask = self.modal_mask_transform(ct_tensor, text_tensor, gene_tensor, modal_mask)

        return (ct_tensor, text_tensor.contiguous(), gene_tensor.contiguous(), modal_mask.contiguous()), label
...
def filter_valid_samples(
    ...
    for _, row in df_multi.iterrows():
        ...
        if sample["modal_present"]["ct"] and sample["label"] is not None:
            valid_samples.append(sample)
...
if __name__ == "__main__":
    ...
    # 2. 构建多模态Dataset
    train_transform  = Compose([RandFlip(),RandRotate(range_x=[-20, -10, -5, 5, 10, 20])])
    ...
    train_dataset = MultiModalDataset(
        valid_samples=[sample for sample in dataset.valid_samples
